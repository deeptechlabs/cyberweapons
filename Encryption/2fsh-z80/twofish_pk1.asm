;; Partial Keying 128 bit Twofish in ECB Mode
;;
...
;;	  inc de, etc.
;;
;;	* Total size is 2782 bytes:  1758 code & 1024 tables
;;
;;	* Encrypts at about 25.0 ms/block @ 4Mhz after key setup
;;
;;	* Code size can be SIGNIFICANTLY decreased by turning macro calls
;;        into subroutines.  MDS multiply can easily be optimized as well.
;;	  Its probably faster to feed input through the Sboxes in parallel...  
;;
;;
...
; Note that MyText is at location 1800H

MyText:		DEFS   16

		;remove defs and put yer test vectors here, eg
...
%macro	RoundFunc	0
	...
	copy4	RESULT1, MyText		; copy input word
	ld	ix, RESULT1		; get ready to operate on it
	ld	iy, S			; setup XOR material
	call	f32			; do it
	
	; Feed the second input word to g function, storing result in RESULT2

	copy4 RESULT2, MyText+4		; copy second input word
	ld	ix, RESULT2	
	ld	iy, S
	ld	b, 08H			; going to rotate 8 bits left first
	call	rol32			; rotate it
	call	f32			; do it
...
;;; ==========================================================================
;;;	EncryptBlock		(PARTIAL KEYING, ECB VERSION)
;;;	Args:  None
...
;; ends just before 22EOH

;;; =========================================================================
;;;	User code
;;; =========================================================================

ORG 3600H			; or wherever
