...
transposed using the same password.  Although a permutation of bytes is a
linear operation, the data-dependent permutation is a non-linear operation
and adds strength to the resulting cipher if used in combination with
others (as in des|tran|des|tran|des).
