...
*			Copyright Peter Gutmann and Stuart Woolford 1996-1999			*
...
void fastPoll( void )
	...
	addRandomString( buffer, &bufIndex, querybuffer, sizeof( querybuffer ) );
	DosGetDateTime( &dt );
	addRandomString( buffer, &bufIndex, &dt, sizeof( DATETIME ) );

	/* Process and Thread information */
	DosGetInfoBlocks( &ptib, &ppib );
	addRandom( buffer, &bufIndex, ptib );
	addRandom( buffer, &bufIndex, ppib );
	addRandomString( buffer, &bufIndex, ptib, sizeof( TIB ) );  /* TIB */
	addRandomString( buffer, &bufIndex, ppib, sizeof( PIB ) );  /* PIB */
	addRandomString( buffer, &bufIndex, ptib->tib_ptib2, sizeof( TIB2 ) ); /* TIB2 */
	...
	if( !addedFixedItems )
		...
		addRandomString( buffer, &bufIndex, ppib->pib_pchcmd, 
						 strlen( ppib->pib_pchcmd ) );
		addRandomString( buffer, &bufIndex, ppib->pib_pchenv, 
    ...
    if (!rc) {
        addRandom( buffer, &bufIndex, procstat );
        addRandomString( buffer, &bufIndex, procstat, sizeof( qsPtrRec_t ) );
    }
...
void slowPoll( void )
	...
	while( ( hwndNext = WinGetNextWindow( henum ) ) != NULLHANDLE )
		{
		addRandom( buffer, &bufIndex, hwndNext );

		WinQueryWindowProcess( hwndNext, &pib, &tib );
		addRandomString( buffer, &bufIndex, &tib, sizeof( TIB ) );
		addRandomString( buffer, &bufIndex, &pib, sizeof( PIB ) );
		WinQueryWindowRect( hwndNext, &rcl );
		addRandomString( buffer, &bufIndex, &rcl, sizeof( RECTL ) );
		}
