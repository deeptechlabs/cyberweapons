...
*						Copyright Peter Gutmann 1994-1997					*
*																			*
****************************************************************************/
...
int rc4Init( CRYPT_INFO *cryptInfo, void *cryptInfoEx )
	{
	int status;
	...
	if( cryptInfo->ctxConv.key != NULL )
		return( CRYPT_INITED );
	if( ( status = krnlMemalloc( &cryptInfo->ctxConv.key, RC4_EXPANDED_KEYSIZE ) ) != CRYPT_OK )
		return( status );
	cryptInfo->ctxConv.keyLength = RC4_EXPANDED_KEYSIZE;
...
int rc4Encrypt( CRYPT_INFO *cryptInfo, BYTE *buffer, int noBytes )
	{
	RC4( ( RC4_KEY * ) cryptInfo->ctxConv.key, noBytes, buffer, buffer );
...
int rc4InitKey( CRYPT_INFO *cryptInfo, const void *key, const int keyLength )
	...
	RC4_set_key( ( RC4_KEY * ) cryptInfo->ctxConv.key, keyLength, 
				 ( BYTE * ) key );
	return( CRYPT_OK );
	}
