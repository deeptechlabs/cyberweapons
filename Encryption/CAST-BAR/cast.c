...
#define REFA
"\xEE\xA9\xD0\xA2\x49\xFD\x3B\xA6\xB3\x43\x6F\xB8\x9D\x6D\xCA\x92"
#define REFB
"\xB2\xC9\x5E\xB0\x0C\x31\xAD\x71\x80\xAC\x05\xB8\xE8\x3D\x69\x6E"
...
void main( int argc, char **argv ) {
   int i, iter, maintflag = 0 ;
   long int j, maint_iter ;
   BYTE *data ;
...
CAST_BLOCKSIZE ], bR[ CAST_BLOCKSIZE ], ref[ CAST_KEYSIZE ] ;
   ...
   if ( argc == 1 ) {
      ...
      printf( "   n (integer) will encrypt n times (useful for timing
tests) \n" ) ;
      printf( "   -x will test the code by encrypting once (and print out
the reference vector) \n" ) ;
      printf( "       and then run the 1000000 iteration Full Maintenance
Test (with reference output)\n" ) ;
      printf( "   -y will test the code by encrypting once (and print out
the reference vector) \n" ) ;
      printf( "       and then run a shortened 1000 iteration Partial
Maintenance Test (with reference output)\n" ) ;
      printf( "       ( which is more convenient for debugging than the
Full test \n" ) ;
