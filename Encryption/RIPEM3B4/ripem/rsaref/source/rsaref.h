...
/* Encryption algorithms.  Some algorithms, like RX2 are "ored" with
     parameters.  The algorithm is the low-order 6 bits (allowing for
     up to 63 algorithms), with the higher order bits containing the
     algorithm parameters (if any).
   The macro EA_GET_ALGORITHM will return the low-order 6 bits.
 */
#define EA_GET_ALGORITHM(algorithm) ((algorithm) & 0x3f)
#define EA_DES_CBC 1
#define EA_DES_EDE2_CBC 2
#define EA_DES_EDE3_CBC 3
#define EA_DESX_CBC 4
/* Use the EA_RX2_CBC macro to specify RX2 with the effective key bits. */
#define EA_RX2_CBC_TOKEN 5
#define EA_RX2_CBC(effectiveBits) \
  (EA_RX2_CBC_TOKEN | ((effectiveBits) << 6))
#define EA_GET_RX2_EFFECTIVE_BITS(algorithm) (((unsigned int)(algorithm)) >> 6)
/* Use the EA_RC5_32_16_CBC macro to specify RC5 (with 32 bit word size
     and 16 rounds) with the key bytes parameter. */
#define EA_RC5_32_16_CBC_TOKEN 6
#define EA_RC5_32_16_CBC(keyBytes) \
  (EA_RC5_32_16_CBC_TOKEN | ((keyBytes) << 6))
#define EA_GET_RC5_KEY_BYTES(algorithm) ((algorithm) >> 6)
...
/* Max digest length is 20 for DA_SHA1 */
...
typedef struct {
  int encryptionAlgorithm;         /* Can include parameter bits, as for RX2 */
  union {
    DES_CBC_CTX des;
    DES3_CBC_CTX des3;
    DESX_CBC_CTX desx;
    RX2_CBC_CTX rx2;
    RC5_32_CBC_CTX rc5_32;
  } cipherContext;
