...
def computeEncryptionKey(password, dictOwnerPass, dictUserPass, dictOE, dictUE, fileID, pElement, dictKeyLength = 128, revision = 3, encryptMetadata = False, passwordType = None):
	...
	'''
	if int(pElement) < -2147483648 or int(pElement) > 2147483647:
		return (-1, '[computeEncryptionKey] Invalid value for pElement (%s)' % str(pElement))
	if revision != 5:
		...
		try:
			md5input = password + dictOwnerPass.value + struct.pack('<i',int(pElement)) + fileID
		except:
		...
		if passwordType == 'USER':
			...
			ret = (-1, '[computeEncryptionKey] passwordType (%s) was not USER or OWNER' % str(passwordType))
		return ret
...
def isUserPass(password, computedUserPass, dictU, revision):
	...
	if revision == 5:
		...
		try:
			if computedUserPass[:16] == dictU.value[:16]:
				return True
			else:
				return False
		except:
