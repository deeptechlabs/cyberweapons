 ...
 * This access the low-level library directly.  It is NOT an
 * example of how to program with the library normally!  By
 * accessing the library at a low level, it is possible to
 * exercise the smallest components and thus localize bugs
 * more accurately.  This is especially useful when writing
 * assembly-language primitives.
 *
 * This also does timing tests on modular exponentiation.
 * Modular exponentiation is so computationally expensive that
 * the fact that this code omits one level of interface glue
 * has no perceptible effect on the results.
 */
...
main(int argc, char **argv)
{
	...
	long modbits = 0, expbits = 0, expbits2 = 0;
	char *p;
...
#define F BIGLITTLE((f+SIZE),f)
	...
	/*
	 * Parse the command line.  The do{} loop is a dummy, to have
	 * something to break out of when done with the command line.
	 */
	do {
		if (argc <= 1)
			break;
		modbits = strtol(argv[1], &p, 0);
		if (modbits < 3 || modbits > MAXBITS || *p) {
			fprintf(stderr, "Illegal modbits: \"%s\"\n", argv[1]);
			return usage(argv[0]);
		}
		if (argc <= 2) {
			expbits2 = expbits = modbits;
			break;
		}
		expbits = strtol(argv[2], &p, 0);
		if (expbits < 3 || expbits > MAXBITS || *p) {
			fprintf(stderr, "Illegal expbits: \"%s\"\n", argv[2]);
			return usage(argv[0]);
		}
		if (argc <= 3) {
			expbits2 = expbits;
		...
		expbits2 = strtol(argv[3], &p, 0);
		if (expbits2 < 3 || expbits2 > MAXBITS || *p) {
			fprintf(stderr, "Illegal expbits2: \"%s\"\n", argv[3]);
			return usage(argv[0]);
		}
		if (argc <= 4)
			break;
		fprintf(stderr, "Unexpected fourth argument \"%s\"\n",
		        argv[4]);
		return usage(argv[0]);
	} while (0);
	...
	if (modbits) {
		...
		for (j = 0; j < 10; j++) {
			...
			bnput16("a = ", A, i);
			z = lbnExpMod_16(A, A, i, C, i, B, i);
			if (z < 0)
				goto nomem;
			bnput16("a^(p-1) mod p = ", A, i);
		}
		...
		for (j = 0; j < 25; j++) {
			...
			*(BIGLITTLE(E-l,E+l-1)) |= (t >> 1) + 1;
			...
			printf("2^<%d>:%3lu.%03u   ",
			       (int)expbits, cursec, curms);
#else
			printf("2^<%d>    ", (int)expbits);
#endif
			fflush(stdout);
			...
			printf("<%d>^<%d>:%3lu.%03u   ",
			       (int)modbits, (int)expbits, cursec, curms);
#else
			printf("<%d>^<%d>    ", (int)modbits, (int)expbits);
#endif
			fflush(stdout);
...
#if CLOCK_AVAIL
		putchar('\n');
		twoexpms += 1000 * (unsigned)(twoexpsec % j);
		while (twoexpms > 1000*j) {
			twoexpsec += j;
			twoexpms -= 1000*j;
		}
		printf("2^<%d> mod <%d> bits AVERAGE: %lu.%03u s\n",
		       (int)expbits, (int)modbits,
		       twoexpsec/j, twoexpms/j);

		expms += 1000 * (unsigned)(expsec % j);
		while (expms > 1000*j) {
			expsec += j;
			expms -= 1000*j;
		}
		printf("<%d>^<%d> mod <%d> bits AVERAGE: %lu.%03u s\n",
		       (int)modbits, (int)expbits, (int)modbits,
		       expsec/j, expms/j);

		dblexpms += 1000 * (unsigned)(dblexpsec % j);
		while (dblexpms > 1000*j) {
			dblexpsec += j;
			dblexpms -= 1000*j;
		}
	...
	for (j = 1; j <= 1000; j++) {
		...
		for (i = 1; i <= SIZE/2; i++) {
			...
			lbnMulN1_16(D, B, i, t);
			memcpy(e, d, sizeof(a));
			carry = *(BIGLITTLE(D-i-1,D+i));
			/* D = A + B * t */
			carry = lbnAddN_16(D,A,i);

			/* For comparison with the result of lbnMulAdd1_16 */
			borrow = carry + *(BIGLITTLE(D-i-1,D+i));

			*(BIGLITTLE(A-i-1,A+i)) = 0;
			...
			/* Check that multiply and square do the same thing */
			lbnMul_16(C,A,i,A,i);
			lbnSquare_16(D,A,i);
			if (lbnCmp_16(C,D,i+i) || !j) {
				bnput16("a*a = ", C, i+i);
			 ...
			 * Start with A > B, and also place a copy of B
			 * into C.
			 */
