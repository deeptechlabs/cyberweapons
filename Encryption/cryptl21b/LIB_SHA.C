...
/* The SHA_CTX structure used in libsha doesn't include the isSHA field from
   the original cryptlib code, so it's necessary to add this to sha.h when
   using a newer version of libsha */
...
int shaInitEx( CRYPT_INFO *cryptInfo, void *cryptInfoEx )
	...
	/* Set things up for SHA or SHA-1.  Note that the user can't specify the
	   use of SHA, this is only present for backwards-compatibility in
	   internal routines, so all we do is check that cryptInfoEx is
	   non-null */
	if( cryptInfoEx != NULL )
		{
		setSHAinfo( cryptInfo, TRUE );
		SHA_Init( ( SHA_CTX * ) cryptInfo->ctxHash.hashInfo );
		}
	else
		{
...
int shaInit( CRYPT_INFO *cryptInfo )
	...
	return( shaInitEx( cryptInfo, NULL ) );
	}
...
int shaHash( CRYPT_INFO *cryptInfo, BYTE *buffer, int noBytes )
	...
	if( !noBytes )
		{
		if( shaInfo->isSHA )
			SHA_Final( cryptInfo->ctxHash.hash, shaInfo );
		else
		...
		{
		if( shaInfo->isSHA )
			SHA_Update( shaInfo, buffer, noBytes );
		else
...
BOOLEAN getSHAinfo( const CRYPT_INFO *cryptInfo )
	{
	return( cryptInfo->ctxHash.algorithmParam1 );
	}

void setSHAinfo( CRYPT_INFO *cryptInfo, const BOOLEAN isSHA )
	{
	cryptInfo->ctxHash.algorithmParam1 = isSHA;
	if( isSHA )
		cryptInfo->ctxHash.nonDefaultValues = TRUE;
	}
