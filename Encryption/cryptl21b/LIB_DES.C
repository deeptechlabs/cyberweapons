...
int desInit( CRYPT_INFO *cryptInfo, void *cryptInfoEx )
	{
	int status;
	...
	if( cryptInfo->ctxConv.key != NULL )
		return( CRYPT_INITED );
	if( ( status = krnlMemalloc( &cryptInfo->ctxConv.key, DES_EXPANDED_KEYSIZE ) ) != CRYPT_OK )
		return( status );
	cryptInfo->ctxConv.keyLength = DES_EXPANDED_KEYSIZE;
