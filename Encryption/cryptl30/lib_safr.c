...
int saferInit( CRYPT_INFO *cryptInfo )
	...
	/* Allocate memory for the key within the crypt context and set up any
	   pointers we need.  We don't handle the number of rounds at this point 
	   since we set things up when we perform the saferInitKey() function, as 
	   the number of rounds is key-dependant */
...
int saferInitKey( CRYPT_INFO *cryptInfo, const void *key, const int keyLength )
	...
	if( shortKey )
		...
		memcpy( cryptInfo->ctxConv.userKey + bitsToBytes( 64 ), cryptInfo->ctxConv.userKey,
				bitsToBytes( 64 ) );
	saferExpandKey(	saferKey, cryptInfo->ctxConv.userKey, currentRounds, TRUE );
