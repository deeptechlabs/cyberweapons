/*
** Insitutional File System Project
** University of Michigan
*/
#ifndef NOSCCSID
#ifndef lint
static char *sccsid = "@(#)verify.c	1.1 3/22/89 12:21:32 (/afs/ifs.umich.edu/usr/andy/des/SCCS/s.verify.c)";
#endif
#endif
/*
 * $Source: /afs/andrew.cmu.edu/usr7/kazar/xxx/des/RCS/verify.c,v $
 * $Author: ota $
 *
...
"$Header: verify.c,v 1.3 89/01/27 14:33:49 ota Exp $";
#endif	lint
    ...
    char *argv[];
{
    ...
    des_cbc_encrypt(input,(des_cblock *) cipher_text,(long) in_length,KS,ivec,1);
    ...
    des_cbc_encrypt((C_Block *) cipher_text,(C_Block *) clear_text,(long) in_length,KS,ivec,0);
...
{
    for (i =1; i<=nflag; i++) {
	des_ecb_encrypt((C_Block *) in,(C_Block *) out,KS,1);
	if (des_debug) {
	    printf("\nclear %s\n",in);
	    for (j = 0; j<=7; j++)
...
{
    for (i =1; i<=nflag; i++) {
	des_ecb_encrypt((C_Block *) out,(C_Block *) in,KS,0);
	if (des_debug) {
	    printf("clear %s\n",in);
	    for (j = 0; j<=7; j++)
