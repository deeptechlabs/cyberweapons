/*
  ...
  9.47 Algorithm MDC-4 (DES-based)
  See "Handbook of Applied Cryptography"
  by Alfred J. Menezes et al page 343.
...
void MDC_4(char *buffer, long length, long *G0, long *G0T)
/* MDC-4 */
{
  char *padded;
  long C[2], CT[2], K[ROUNDS][2], KT[ROUNDS][2], x[2];
  long D[2], DT[2], H0[2], H0T[2];
  long i, t = (length >> LOG_N_BYTES) + 1;

  padded = calloc(t * BYTES_PER_BLOCK, sizeof(char));
  memcpy(padded, buffer, length);
  padded[length] = 0x80;
  for (i = 0; i < t; i++) {
    memcpy(x, padded + i * BYTES_PER_BLOCK, BYTES_PER_BLOCK);
    DES_key_schedule(K, G0);
    DES_key_schedule(KT, G0T);
    DES(K, x, C);
    DES(KT, x, CT);
    C[0] ^= x[0];
    C[1] ^= x[1];
...
int main(void)
{
  ...
  long G0[2], G0T[2], G0_bit[64], G0T_bit[64];
  long I0[2], I0T[2], I0_bit[64], I0T_bit[64];
  long count = 0, i, length = 26;

  srand(time(NULL));
  G0[0] = I0[0] = (rand() << 16) | rand();
  G0[1] = I0[1] = (rand() << 16) | rand();
  G0T[0] = I0T[0] = (rand() << 16) | rand();
  G0T[1] = I0T[1] = (rand() << 16) | rand();
  printf("%s\n", buffer);
  printf("before G0  = %8x %8x\n", G0[0], G0[1]);
  printf("before G0~ = %8x %8x\n", G0T[0], G0T[1]);
  MDC_4(buffer, length, G0, G0T);
  printf("after  G0  = %8x %8x\n", G0[0], G0[1]);
  printf("after  G0~ = %8x %8x\n", G0T[0], G0T[1]);
  buffer[0] -= (char) 1;
  printf("%s\n", buffer);
  printf("before G0  = %8x %8x\n", I0[0], I0[1]);
  printf("before G0~ = %8x %8x\n", I0T[0], I0T[1]);
  MDC_4(buffer, length, I0, I0T);
  printf("after  G0  = %8x %8x\n", I0[0], I0[1]);
  printf("after  G0~ = %8x %8x\n", I0T[0], I0T[1]);
  for (i = 1; i <= 64; i++) {
    G0_bit[i] = get_bit(i, G0);
    I0_bit[i] = get_bit(i, I0);
    if (G0_bit[i] != I0_bit[i]) count++;
    G0T_bit[i] = get_bit(i, G0T);
    I0T_bit[i] = get_bit(i, I0T);
    if (G0T_bit[i] != I0T_bit[i]) count++;
  }
