/*
  ...
  9.46 Algorithm MDC-2 (DES-based)
  See "Handbook of Applied Cryptography"
  by Alfred J. Menezes et al pages 342
  and 343. The message to be hashed is
  assumed to have a bitlength that is
...
void MDC_2(char *buffer, long length, long *H0, long *H0T)
/* MDC-2 */
{
  char *padded;
...
int main(void)
{
  ...
  long H0[2], H0T[2], H0_bit[64], H0T_bit[64];
  long I0[2], I0T[2], I0_bit[64], I0T_bit[64];
  long count = 0, i, length = 26;

  H0[0] = I0[0] = 0x52525252;
  H0[1] = I0[1] = 0x52525252;
  H0T[0] = I0T[0] = 0x25252525;
  H0T[1] = I0T[1] = 0x25252525;
  printf("%s\n", buffer);
  printf("before H0  = %8x %8x\n", H0[0], H0[1]);
  printf("before H0~ = %8x %8x\n", H0T[0], H0T[1]);
  MDC_2(buffer, length, H0, H0T);
  ...
  for (i = 1; i <= 64; i++) {
    ...
    H0T_bit[i] = get_bit(i, H0T);
    I0T_bit[i] = get_bit(i, I0T);
    if (H0T_bit[i] != I0T_bit[i]) count++;
  }
