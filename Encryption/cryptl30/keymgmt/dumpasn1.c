...
static const char *configPaths[] = {
	...
	/* It's my program, I'm allowed to hardwire in strange paths which noone 
	   else uses */
	"c:\\program files\\bin\\",
...
void usageExit( void )
	...
	puts( "Copyright Peter Gutmann 1997 - 2000.  Last updated 22 February 2000." );
	puts( "" );
	puts( "Usage: dumpasn1 [-abcdfhlopsxz] <file>" );
	...
	while( argc && *argv[ 0 ] == '-' && moreArgs )
		...
		while( *argPtr )
			...
			switch( toupper( *argPtr ) )
				...
				case 'S':
					/* Not sure how portable this is, but it seems to be
					   better than output = NULL which fails under Linux */
					output = stdin;
					break;
