 ...
 */
void skipjackMakeKey(byte key[10], byte tab[10][256]) {
 ...
 */
void skipjackEncrypt(byte tab[10][256], byte in[8], byte out[8]) {
	word32 w1, w2, w3, w4;
 ...
 */
void skipjackDecrypt(byte tab[10][256], byte in[8], byte out[8]) {
	word32 w1, w2, w3, w4;
...
int main() {
	...
	skipjackMakeKey(key, tab);
	skipjackEncrypt(tab, inp, enc);
	printf((memcmp(enc, chk, 8) == 0) ? "encryption OK!\n" : "encryption failure!\n");
	skipjackDecrypt(tab, enc, dec);
