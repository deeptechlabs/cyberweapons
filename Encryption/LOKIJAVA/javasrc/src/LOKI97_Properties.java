 ...
 */
public class LOKI97_Properties // implicit no-argument constructor
{
// Constants and variables with relevant static code
//...........................................................................

    static final String ALGORITHM = "LOKI97";
    static final double VERSION = 1.0;
    static final String FULL_NAME = ALGORITHM + " ver. " + VERSION;
    static final String NAME = "LOKI97_Properties";

    static final boolean GLOBAL_DEBUG = true;
    static final boolean TRACE = false;                 // trace prop & alg load

    static final Properties properties = new Properties();

    /** Default properties in case .properties file was not found. */
    private static final String[][] DEFAULT_PROPERTIES = {
        {"Trace.LOKI97_Algorithm",       "true"},
        {"Debug.Level.*",                "1"},
        {"Debug.Level.LOKI97_Algorithm", "9"},
    };

    static {
if (TRACE) System.out.println(">>> " + NAME + ": Looking for " + ALGORITHM + " properties");
        String it = ALGORITHM + ".properties";
        InputStream is = LOKI97_Properties.class.getResourceAsStream(it);
        boolean ok = is != null;
        if (ok)
            try {
                properties.load(is);
                is.close();
if (TRACE) System.out.println(">>> " + NAME + ": Properties file loaded OK...");
            } catch (Exception x) {
                ok = false;
            }
        if (!ok) {
if (TRACE) System.out.println(">>> " + NAME + ": WARNING: Unable to load \"" + it + "\" from CLASSPATH.");
if (TRACE) System.out.println(">>> " + NAME + ":          Will use default values instead...");
            int n = DEFAULT_PROPERTIES.length;
            for (int i = 0; i < n; i++)
                properties.put(
                    DEFAULT_PROPERTIES[i][0], DEFAULT_PROPERTIES[i][1]);
if (TRACE) System.out.println(">>> " + NAME + ": Default properties now set...");
        }
    }
