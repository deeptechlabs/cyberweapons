 ...
 */
#ifdef SMALL_CACHE
CONST static uint16 mul(a, b)
register uint16 a; register uint16 b;
{
	register word32 p;
...
register byteptr buf; register byteptr mask;
register int count;
/*	count must be > 0 */
{
	if (count) 
...
static void initkey_idea(key, decryp)
...
*/
static void idea_ecb(inbuf, outbuf)
