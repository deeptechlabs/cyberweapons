...
int hmacMD5SelfTest( void )
	...
	cryptInfo.ctxMAC.macInfo = &md5Info;
	...
	for( i = 0; hmacValues[ i ].data != NULL; i++ )
		...
		MD5_Init( ( MD5_CTX * ) cryptInfo.ctxMAC.macInfo );
	...
	return( ( hmacValues[ i ].data == NULL ) ? CRYPT_OK : CRYPT_SELFTEST );
	}
...
int hmacMD5Hash( CRYPT_INFO *cryptInfo, BYTE *buffer, int noBytes )
	{
	MD5_CTX *md5Info = ( MD5_CTX * ) cryptInfo->ctxMAC.macInfo;
