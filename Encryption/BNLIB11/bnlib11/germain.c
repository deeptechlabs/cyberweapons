...
#if BNDEBUG
/*
 * For sanity checking the sieve, we check for small divisors
 * of the numbers we get back.  This takes "rem", a partially
 * reduced form of the prime, "div" a divisor to check for,
 * and "order", a parameter of the "order" of Sophie Germain
 * primes (0 = normal primes, 1 = Sophie Germain primes,
 * 2 = 4*p+3 is also prime, etc.) and does the check.
 * It just complains to stdout if the check fails.
 */
static void
germainSanity(unigned rem, unsigned div, unsigned order)
{
	unsigned mul = 1;
...
#define CONFIRMTESTS 7
	...
	for (i = j; i < sizeof(primes)/sizeof(*primes); i++) {
		...
		/* Final success indicator for the base primes[i]. */
		if (f && (err = f(arg, '*')) < 0)
			return err;
	}
 ...
 *
 * This is actually parameterized: it generates primes p such that "dbl"
 ...
 */
int
germainPrimeGen(struct BigNum *bn, unsigned dbl,
	int (*f)(void *arg, int c), void *arg)
{
	...
	if (dbl) {
		/*
		 * Obviously, the prime we find must be odd.  Further,
		 * p != 0 (mod 3), and 2*p+1 != 0 (mod 3), meaning that
		 * p != 1 (mod 3).  Thus, p == 2 (mod 3), and p odd
		 * and that p == 5 (mod 6).  Increase bn to have this
		 * property, and search is steps of 6.
		 */
		if (bnAddQ(bn, 5 - bnModQ(bn, 6)) < 0)
			goto failed;
		inc = 6;
	} else {
		(void)bnAddQ(bn, ~bnLSWord(bn) & 1);	/* Round up to odd */
		inc = 2;
	}
		

	for (;;) {
		if (sieveBuild(sieve, SIEVE, bn, inc, dbl) < 0)
			goto failed;
...
#endif
	return retval < 0 ? retval : modexps+(dbl+1)*CONFIRMTESTS;
}
